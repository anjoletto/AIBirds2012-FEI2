...
/**
 * Test class for Trajectory_Regression/
 *
 * @author Kar Wai Lim
 *
...
public class Trajectory_Test {
	...
	private static APoint normalise(APoint a, APoint ref) {
		APoint result = new APoint(ref.getX() - a.getX(), ref.getY() - a.getY());
		return result;
	}
	
	////
	private static double predict(double x, double xStart, double yStart, double para1, double para2) {
		return para1*x*yStart/xStart + para2*x*x/(xStart*xStart);
	}
